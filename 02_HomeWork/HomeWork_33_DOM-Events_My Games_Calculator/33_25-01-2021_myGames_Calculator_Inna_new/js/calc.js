...
function calc(){
  let reducer;
  for(item of array){ // (i=0; i<array.length; i++) цикл с индексами не работал как надо на втором круге (когда после нажатия кнопки = хотелось выполнить ещё одну операцию). В этом случае ловился правильный символ и вызывался соответствующий метод, но в строчке 38 массив сплитился уже не по этому символу, а по числу и получалось ложное вычисление.
    if(isNaN(item) && item !== '.'){
      for(key in methods){
        if(key == item)
        reducer = methods[`${key}`];
