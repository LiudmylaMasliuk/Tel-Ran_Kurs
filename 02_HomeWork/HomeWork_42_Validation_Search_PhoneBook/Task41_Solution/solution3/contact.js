...
class ContactsList{
    ...
    renderContactsList(){
        ...
        const listOfContacts = this;
        contactsListView.onclick = function(e){
        //console.log(e.target);
        let li = e.target;
        if(li.classList.contains('delete')){
             li.classList.remove('item-active');
             listOfContacts.deleteContact(li.dataset.index);
             listOfContacts.renderContactsList();
        }else{
            if(li.tagName !== 'LI'){
                li= e.target.parentNode;
            }
            root.querySelectorAll('li').forEach((item)=>item.classList.remove('item-active'));
            const contact = listOfContacts.list[li.dataset.index];
            contact.renderContactView();
            li.classList.add('item-active')
        }
    }
    listOfContacts.list.forEach(function(item, index){
