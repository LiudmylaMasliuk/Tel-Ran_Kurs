class Contact{
  constructor(name,phone,email,address,description, parentSelector = '#root'){
    this.name = name;
    this.phone = phone;
    this.email = email;
...
class  ContactsList{
  ...
  addContact(contact){
    const tmp = this.list.find(function(item){ // v tmp sohranitsja true ili false
      return item.name === contact.name;
    });
    if(!tmp){ // !false (=true) // esli ne nawel, to etot kontakt dobav v massiv
  ...
  renderContactsList(){
    ...
    root.innerHTML = '';
    root.innerHTML += `<div class = "contact-view">No selector contact</div>`;
